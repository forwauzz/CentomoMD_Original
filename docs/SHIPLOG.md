...
## 2025-01-11 21:30 — [fix/ambient-listening-troubleshoot] feat(ambient): implement comprehensive speaker diarization for Mode 3 - add explicit AWS Transcribe parameters (ShowSpeakerLabel, MaxSpeakerLabels, ChannelIdentification) for each mode, implement runtime validation guards and STREAM_PARAMS logging for verification, enhance finalizeAmbientSession with detailed diarization summary and FINAL_STREAMING_SUMMARY logging, add diarization validation to /api/transcribe/process endpoint with proper error handling, update frontend useTranscription to show toast notifications for diarization status, add ambientDebugMode feature flag with 'View raw JSON' debug button for troubleshooting, fix frontend mode parameter timing issues in useTranscription hook, ensure Mode 3 pipeline properly processes speaker-labeled transcripts, add comprehensive logging and validation throughout the speaker diarization pipeline, speaker diarization now working with clear numeric labels (0:, 1:, 2:) distinguishing between speakers in medical consultations — commit:de41cb0
