...
// Save or update case endpoint (handles both create and update)
app.post('/api/cases/save-or-update', async (req, res) => {
  try {
    const { case_id, draft, user_id, clinic_id } = req.body;
    ...
    if (case_id) {
      ...
      if (existingCase.length > 0 && existingCase[0].user_id === mockUserId) {
        ...
        console.log('✅ Case updated successfully:', result[0]);
      } else {
        // Case doesn't exist or doesn't belong to user, create new one
        ...
        console.log('✅ New case created (case_id not found):', result[0]);
      }
    } else {
      // No case_id provided, create new case
      result = await db.insert(cases).values({
        user_id: mockUserId,
        clinic_id: mockClinicId,
        draft: draft,
      ...
      console.log('✅ New case created (no case_id provided):', result[0]);
    }
    ...
    res.json({
      ...
      data: {
        uid: result[0].uid,
        message: case_id ? 'Case updated successfully' : 'Case created successfully'
      }
    });

  } catch (error) {
    console.error('❌ Error saving/updating case:', error);
    res.status(500).json({ 
      success: false, 
      error: 'Failed to save/update case',
      details: error instanceof Error ? error.message : 'Unknown error'
    });
  }
});

// Create empty case endpoint
app.post('/api/cases/create', async (req, res) => {
  try {
    ...
    // Insert empty case in database to get a case ID
    const result = await db.insert(cases).values({
      user_id: mockUserId,
      clinic_id: mockClinicId,
      draft: null, // Empty case, no draft data yet
      updated_at: new Date()
    }).returning();

    console.log('✅ Empty case created successfully:', result[0]);

    res.json({
      success: true,
      data: {
        uid: result[0].uid,
        message: 'Empty case created successfully'
      }
    });

  } catch (error) {
    console.error('❌ Error creating empty case:', error);
    res.status(500).json({ 
      success: false, 
      error: 'Failed to create empty case',
      details: error instanceof Error ? error.message : 'Unknown error'
    });
  }
});
